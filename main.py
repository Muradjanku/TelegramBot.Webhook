...
def scrape_aros_catalog():
    try:
        ...
        products = []
        # Misol uchun, mahsulotlar ro'yxatini topish (sayt strukturasiga moslashtiriladi)
        product_elements = soup.select('.product-item')  # Aros.uz saytidagi elementlarni moslashtiring
        ...
        ''')
        cursor.executemany('INSERT OR IGNORE INTO aros_products (name, price, link) VALUES (?, ?, ?)',
...
async def button_callback(update: Update, _: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("aros_"):
        ...
        conn = sqlite3.connect('products.db')
        cursor = conn.cursor()
        cursor.execute('SELECT name, price, link FROM aros_products')
